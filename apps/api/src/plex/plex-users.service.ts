...
export class PlexUsersService {
  ...
  async getAdminPlexUser(): Promise<PlexUserRow> {
  ...
  async getPlexUserById(id: string): Promise<PlexUserRow | null> {
  ...
  async getOrCreateByPlexAccount(
    params: PlexAccountIdentity,
  ): Promise<PlexUserRow | null> {
  ...
  async ensureAdminPlexUser(params: { userId?: string | null }) {
  ...
  async backfillAdminOnMissing(): Promise<void> {
  ...
  private async createAdminPlaceholder(): Promise<PlexUserRow> {
    try {
      return await this.prisma.plexUser.create({
        data: {
          id: DEFAULT_ADMIN_ID,
  ...
  private async getFirstAdminUserId(): Promise<string | null> {
  ...
  private async getAdminIdentity(
    userId: string | null,
  ): Promise<PlexAccountIdentity | null> {
    ...
    try {
      ...
      this.logger.warn(
        `Plex whoami failed while ensuring admin user: ${(err as Error)?.message ?? String(err)}`,
      );
      return null;
    }
  }

  private async mergePlexUserData(params: { fromId: string; toId: string }) {
