...
exports.ImmaculateTasteCollectionJob = void 0;
...
function normalizeAndCapTitles(rawTitles, max) {
    ...
    for (const raw of rawTitles ?? []) {
        const t = (0, title_normalize_1.normalizeTitleForMatching)(String(raw ?? '').trim());
        if (!t)
            continue;
...
function buildLibrarySelectionSkippedReport(params) {
    const reasonMessage = params.reason === 'library_excluded'
        ...
        tasks: [
            {
                ...
                issues: [(0, job_report_v1_1.issue)('warn', reasonMessage)],
            },
        ],
        issues: [(0, job_report_v1_1.issue)('warn', reasonMessage)],
        raw: {
...
let ImmaculateTasteCollectionJob = class ImmaculateTasteCollectionJob {
    prisma;
    settingsService;
    plexServer;
    plexUsers;
    ...
    constructor(prisma, settingsService, plexServer, plexUsers, recommendations, tmdb, radarr, sonarr, immaculateTaste, immaculateTasteTv, immaculateTasteRefresher) {
        this.prisma = prisma;
        ...
        this.immaculateTasteRefresher = immaculateTasteRefresher;
    }
    async run(ctx) {
        ...
        const pinTarget = pinCollections
            ? 'admin'
            : 'friends';
        const mediaTypeRaw = typeof input['mediaType'] === 'string' ? input['mediaType'].trim() : '';
        const mediaType = mediaTypeRaw.toLowerCase();
        const isTv = mediaType === 'episode' || mediaType === 'show' || mediaType === 'tv';
        const resolvedMediaType = isTv ? 'tv' : 'movie';
        void ctx
            .patchSummary({ mediaType: resolvedMediaType })
            .catch(() => undefined);
        const seedTitleRaw = typeof input['seedTitle'] === 'string' ? input['seedTitle'].trim() : '';
        const seedTitle = (0, title_normalize_1.normalizeTitleForMatching)(seedTitleRaw);
        const seedRatingKey = typeof input['seedRatingKey'] === 'string'
            ? input['seedRatingKey'].trim()
            : '';
        const seedLibrarySectionIdRaw = typeof input['seedLibrarySectionId'] === 'number' &&
            ...
            : typeof input['seedLibrarySectionId'] === 'string'
                ...
                : '';
        const seedLibrarySectionTitle = typeof input['seedLibrarySectionTitle'] === 'string'
            ? input['seedLibrarySectionTitle'].trim()
            : '';
        const seedYear = typeof input['seedYear'] === 'number' &&
        ...
        void ctx
            ...
            .catch(() => undefined);
        const sections = await (0, job_retry_1.withJobRetry)(() => this.plexServer.getSections({
            baseUrl: plexBaseUrl,
            token: plexToken,
        ...
        if (!movieSectionKey && seedRatingKey) {
            const meta = await (0, job_retry_1.withJobRetryOrNull)(() => this.plexServer.getMetadataDetails({
                baseUrl: plexBaseUrl,
                token: plexToken,
                ratingKey: seedRatingKey,
            }), { ctx, label: 'plex: get seed metadata', meta: { ratingKey: seedRatingKey } });
            if (meta?.librarySectionId)
                movieSectionKey = meta.librarySectionId;
        ...
        if (movieSectionKey && excludedSectionKeySet.has(movieSectionKey)) {
            await ctx.warn('immaculateTastePoints: skipped (resolved seed library excluded)', {
        ...
        if (!movieSectionKey) {
            const preferred = movieSections.find((s) => s.title.toLowerCase() === 'movies') ??
                movieSections[0];
        ...
        const googleEnabled = googleEnabledFlag &&
            Boolean(googleApiKey) &&
            Boolean(googleSearchEngineId);
        const suggestionsPerRunRaw = pickNumber(settings, 'recommendations.count') ??
            pickNumber(settings, 'immaculateTaste.suggestionsPerRun') ??
            10;
        const suggestionsPerRun = Math.max(5, Math.min(100, Math.trunc(suggestionsPerRunRaw || 10)));
        const upcomingPercentRaw = pickNumber(settings, 'recommendations.upcomingPercent') ?? 25;
        const upcomingPercent = Math.max(0, Math.min(75, Math.trunc(upcomingPercentRaw || 25)));
        const maxPoints = Math.trunc(pickNumber(settings, 'immaculateTaste.maxPoints') ?? 50) || 50;
        const includeRefresherAfterUpdate = pickBool(settings, 'immaculateTaste.includeRefresherAfterUpdate') ?? true;
        const startSearchImmediately = pickBool(settings, 'jobs.immaculateTastePoints.searchImmediately') ?? false;
        const approvalRequiredFromObservatory = pickBool(settings, 'jobs.immaculateTastePoints.approvalRequiredFromObservatory') ??
            false;
        ...
        const requestedCount = Math.min(100, Math.max(suggestionsPerRun, Math.max(1, suggestionsPerRun * 2)));
        const recs = await (0, job_retry_1.withJobRetry)(() => this.recommendations.buildSimilarMovieTitles({
            ctx,
            seedTitle,
            seedYear,
            tmdbApiKey,
        ...
        for (const title of normalizedTitles) {
            ...
            if (!t)
                continue;
            const found = await (0, job_retry_1.withJobRetryOrNull)(() => this.plexServer.findMovieRatingKeyByTitle({
                baseUrl: plexBaseUrl,
                token: plexToken,
                librarySectionKey: movieSectionKey,
                title: t,
        ...
        for (const it of resolved) {
            if (!unique.has(it.ratingKey))
                unique.set(it.ratingKey, it.title);
        }
        ...
        const getVoteStats = async (tmdbId) => {
            const cached = tmdbDetailsCache.get(tmdbId) ?? null;
            if (cached)
                return cached;
            const vote = await (0, job_retry_1.withJobRetryOrNull)(() => this.tmdb.getMovieVoteStats({ apiKey: tmdbApiKey, tmdbId }), { ctx, label: 'tmdb: get movie vote stats', meta: { tmdbId } });
            const normalized = {
        ...
        for (const it of suggestedItems) {
            ...
            if (!tmdbId) {
                const match = await (0, job_retry_1.withJobRetryOrNull)(() => this.pickBestTmdbMatch({ tmdbApiKey, title }), { ctx, label: 'tmdb: resolve movie title', meta: { title } });
        ...
        for (const title of missingTitles) {
            ...
            if (!t)
                continue;
            if (missingTitleToTmdb.has(t))
                continue;
            const match = await (0, job_retry_1.withJobRetryOrNull)(() => this.pickBestTmdbMatch({ tmdbApiKey, title: t }), { ctx, label: 'tmdb: resolve missing movie title', meta: { title: t } });
            if (!match)
                continue;
        ...
        for (const [k, v] of Array.from(missingTitleToTmdb.entries())) {
            if (rejectIds.has(String(v.tmdbId)))
                missingTitleToTmdb.delete(k);
        }
        ...
        const fetchMissingRadarr = pickBool(settings, 'jobs.immaculateTastePoints.fetchMissing.radarr') ??
            true;
        ...
        if (!ctx.dryRun && radarrEnabled && missingTitles.length) {
            if (approvalRequiredFromObservatory) {
                ...
                radarrLists.skipped.push(...missingTitles.map((t) => String(t ?? '').trim()).filter(Boolean));
            }
            else {
                ...
                const defaults = await (0, job_retry_1.withJobRetry)(() => this.pickRadarrDefaults({
                    ctx,
                    ...
                    preferredQualityProfileId: Math.max(1, Math.trunc(pickNumber(settings, 'radarr.defaultQualityProfileId') ??
                        ...
                        1)) || 1,
                    preferredTagId: (() => {
                        const v = pickNumber(settings, 'radarr.defaultTagId') ??
                            pickNumber(settings, 'radarr.tagId');
                        return v && Number.isFinite(v) && v > 0 ? Math.trunc(v) : null;
                    })(),
                }), { ctx, label: 'radarr: resolve defaults', meta: { baseUrl: radarrBaseUrl } }).catch((err) => ({ error: err?.message ?? String(err) }));
                if ('error' in defaults) {
                    await ctx.warn('radarr: defaults unavailable (skipping adds)', defaults);
                }
                else {
                    let radarrIndexByTmdb = null;
                    const ensureRadarrIndex = async () => {
                        if (radarrIndexByTmdb)
                            return radarrIndexByTmdb;
                    ...
                    for (const title of missingTitles) {
                        ...
                        try {
                            const result = await (0, job_retry_1.withJobRetry)(() => this.radarr.addMovie({
                                baseUrl: radarrBaseUrl,
                                apiKey: radarrApiKey,
                            ...
                            if (result.status === 'added') {
                                ...
                                radarrSentTmdbIds.push(tmdbMatch.tmdbId);
                                const idx = await (0, job_retry_1.withJobRetryOrNull)(() => ensureRadarrIndex(), {
                                    ctx,
                                    label: 'radarr: index movies',
                                });
                                const existing = idx ? idx.get(tmdbMatch.tmdbId) ?? null : null;
                                if (existing) {
                                    await (0, job_retry_1.withJobRetry)(() => this.radarr.setMovieMonitored({
                                        baseUrl: radarrBaseUrl,
                                        apiKey: radarrApiKey,
                                        movie: existing,
                                        monitored: true,
        ...
        if (!ctx.dryRun) {
            const now = new Date();
            const missingTmdbIds = Array.from(new Set(Array.from(missingTitleToTmdb.values()).map((v) => v.tmdbId)));
            const activeTmdbIds = Array.from(new Set(suggestedForPoints.filter((s) => s.inPlex).map((s) => s.tmdbId)));
    ...
    async runTv(params) {
        const { ctx, plexUserId, plexUserTitle, pinTarget, seedTitle, seedYear, seedRatingKey, seedLibrarySectionIdRaw, seedLibrarySectionTitle, } = params;
        const { settings, secrets } = await this.settingsService.getInternalSettings(ctx.userId);
        void ctx
            .patchSummary({
            progress: {
        ...
        if (!tvSectionKey && seedRatingKey) {
            ...
            if (meta?.librarySectionId)
                tvSectionKey = meta.librarySectionId;
            if (meta?.librarySectionTitle)
                tvLibraryName = meta.librarySectionTitle;
        }
        if (tvSectionKey && excludedSectionKeySet.has(tvSectionKey)) {
            await ctx.warn('immaculateTastePoints(tv): skipped (resolved seed library excluded)', {
        ...
        if (!tvSectionKey) {
            const preferred = tvSections.find((s) => s.title.toLowerCase() === 'tv shows') ??
            ...
            if (match?.title)
                tvLibraryName = match.title;
            if (match?.type && match.type.toLowerCase() !== 'show') {
                await ctx.warn('plex: seed librarySectionID is not a TV library (continuing)', {
                    tvSectionKey,
        ...
        const requestedCount = Math.min(100, Math.max(suggestionsPerRun, Math.max(1, suggestionsPerRun * 2)));
        const recs = await (0, job_retry_1.withJobRetry)(() => this.recommendations.buildSimilarTvTitles({
            ctx,
            seedTitle,
            seedYear,
            tmdbApiKey,
        ...
        for (const title of normalizedTitles) {
            ...
            if (!t)
                continue;
            const found = await (0, job_retry_1.withJobRetryOrNull)(() => this.plexServer.findShowRatingKeyByTitle({
                baseUrl: plexBaseUrl,
                token: plexToken,
        ...
        const suggestedItems = Array.from(unique.entries()).map(([ratingKey, title]) => ({ ratingKey, title }));
        const resolvedTitles = suggestedItems.map((d) => d.title);
        ...
        const pushSuggested = (match, inPlex) => {
            if (!match.tvdbId)
                return;
            suggestedForPoints.push({
                tvdbId: match.tvdbId,
        ...
        const fetchMissingSonarr = pickBool(settings, 'jobs.immaculateTastePoints.fetchMissing.sonarr') ??
            true;
        ...
        if (!ctx.dryRun && sonarrEnabled && missingTitles.length) {
            if (approvalRequiredFromObservatory) {
                ...
                sonarrLists.skipped.push(...missingTitles.map((t) => String(t ?? '').trim()).filter(Boolean));
            }
            else {
                const defaults = await (0, job_retry_1.withJobRetry)(() => this.pickSonarrDefaults({
                    ctx,
                    ...
                    preferredQualityProfileId: Math.max(1, Math.trunc(pickNumber(settings, 'sonarr.defaultQualityProfileId') ??
                        ...
                        1)) || 1,
                    preferredTagId: (() => {
                        const v = pickNumber(settings, 'sonarr.defaultTagId') ??
                            pickNumber(settings, 'sonarr.tagId');
                        return v && Number.isFinite(v) && v > 0 ? Math.trunc(v) : null;
                    })(),
                }), { ctx, label: 'sonarr: resolve defaults', meta: { baseUrl: sonarrBaseUrl } }).catch((err) => ({ error: err?.message ?? String(err) }));
                if ('error' in defaults) {
                    await ctx.warn('sonarr: defaults unavailable (skipping adds)', defaults);
                }
                else {
                    let sonarrIndexByTvdb = null;
                    const ensureSonarrIndex = async () => {
                        if (sonarrIndexByTvdb)
                            return sonarrIndexByTvdb;
                    ...
                    for (const title of missingTitles) {
                        ...
                        try {
                            const result = await (0, job_retry_1.withJobRetry)(() => this.sonarr.addSeries({
                                baseUrl: sonarrBaseUrl,
                                apiKey: sonarrApiKey,
                                title: ids.title,
                                tvdbId,
                            ...
                            if (result.status === 'added') {
                                ...
                                sonarrSentTvdbIds.push(tvdbId);
                                const idx = await (0, job_retry_1.withJobRetryOrNull)(() => ensureSonarrIndex(), {
                                    ctx,
                                    label: 'sonarr: index series',
                                });
                                const existing = idx ? idx.get(ids.tvdbId) ?? null : null;
                                if (existing && existing.monitored === false) {
                                    await (0, job_retry_1.withJobRetry)(() => this.sonarr.updateSeries({
                                        baseUrl: sonarrBaseUrl,
                                        apiKey: sonarrApiKey,
        ...
        if (!ctx.dryRun) {
            const now = new Date();
            const missingTvdbIds = Array.from(new Set(Array.from(missingTitleToIds.values()).map((v) => v.tvdbId).filter((x) => Boolean(x))));
            const activeTvdbIds = Array.from(new Set(suggestedForPoints.filter((s) => s.inPlex).map((s) => s.tvdbId)));
    ...
    async pickRadarrDefaults(params) {
        ...
        const qualityProfile = qualityProfiles.find((p) => p.id === desiredQualityId) ??
            (desiredQualityId !== 1
                ? qualityProfiles.find((p) => p.id === 1)
                : null) ??
            qualityProfiles[0];
        const preferredTagId = typeof params.preferredTagId === 'number' &&
    ...
    async pickSonarrDefaults(params) {
        const { ctx, baseUrl, apiKey } = params;
        const [rootFolders, qualityProfiles, tags] = await Promise.all([
    ...
    async pickBestTmdbMatch(params) {
        ...
        return {
            ...
            vote_average: typeof best.vote_average === 'number' &&
                Number.isFinite(best.vote_average)
                ? Number(best.vote_average)
                : null,
            vote_count: typeof best.vote_count === 'number' && Number.isFinite(best.vote_count)
                ? Math.max(0, Math.trunc(best.vote_count))
                : null,
        };
    }
    async pickBestTmdbTvMatch(params) {
        const results = await (0, job_retry_1.withJobRetryOrNull)(() => this.tmdb.searchTv({
            apiKey: params.tmdbApiKey,
            query: params.title,
            includeAdult: false,
        ...
        const best = exact ?? results[0];
        const details = await (0, job_retry_1.withJobRetryOrNull)(() => this.tmdb.getTv({
            apiKey: params.tmdbApiKey,
            tmdbId: best.id,
        ...
        const tvdbId = typeof tvdbIdRaw === 'number' && Number.isFinite(tvdbIdRaw)
            ? Math.trunc(tvdbIdRaw)
            : null;
        const yearRaw = (details?.first_air_date ?? best.first_air_date ?? '').slice(0, 4);
        const year = yearRaw ? Number.parseInt(yearRaw, 10) : NaN;
        ...
        const vote_average = typeof voteAverageRaw === 'number' && Number.isFinite(voteAverageRaw)
            ? Number(voteAverageRaw)
            : null;
        const vote_count = typeof voteCountRaw === 'number' && Number.isFinite(voteCountRaw)
...
exports.ImmaculateTasteCollectionJob = ImmaculateTasteCollectionJob;
exports.ImmaculateTasteCollectionJob = ImmaculateTasteCollectionJob = __decorate([
...
function buildImmaculateTastePointsReport(params) {
    ...
    const refresherObj = refresher && typeof refresher === 'object' && !Array.isArray(refresher)
        ? refresher
        : null;
    const refresherSkipped = refresherObj && typeof refresherObj.skipped === 'boolean' ? refresherObj.skipped : null;
    const refresherReason = refresherObj && typeof refresherObj.reason === 'string' ? refresherObj.reason : null;
    const refresherError = refresherObj && typeof refresherObj.error === 'string'
        ? refresherObj.error.trim()
        : null;
    const refresherCollectionFacts = [];
    const refresherRaw = refresherObj &&
        refresherObj.template === 'jobReportV1' &&
        isPlainObject(refresherObj.raw)
        ? refresherObj.raw
        : null;
    const appendRefresherCollectionFacts = (side, labelPrefix, unit) => {
        const byLibraryRaw = side?.plexByLibrary;
        const byLibrary = Array.isArray(byLibraryRaw)
            ? byLibraryRaw.filter((x) => Boolean(x) && typeof x === 'object' && !Array.isArray(x))
            : [];
    ...
    const issues = [
        ...(radarrFailed ? [(0, job_report_v1_1.issue)('warn', `Radarr: ${radarrFailed} add(s) failed.`)] : []),
        ...(sonarrFailed ? [(0, job_report_v1_1.issue)('warn', `Sonarr: ${sonarrFailed} add(s) failed.`)] : []),
        ...(refresherError ? [(0, job_report_v1_1.issue)('error', `Refresher failed: ${refresherError}`)] : []),
    ];
    const rawMediaType = String(raw.mediaType ?? '')
        .trim()
        .toLowerCase();
    const normalizedMediaType = rawMediaType === 'tv' || rawMediaType === 'movie' ? rawMediaType : '';
    const mode = (normalizedMediaType || (sonarr ? 'tv' : 'movie')) === 'tv' ? 'tv' : 'movie';
    const rawWithMediaType = { ...raw, mediaType: mode };
    const normalizeTitle = (value) => String(value ?? '').trim().toLowerCase();
    const generatedTitles = sortTitles(uniqueStrings(asStringArray(raw.generatedTitles)));
    ...
    const excludedByRejectListTitles = sortTitles(uniqueStrings(asStringArray(raw.excludedByRejectListTitles)));
    const excludedByRejectListCount = asNum(raw.excludedByRejectListCount) ?? excludedByRejectListTitles.length;
    ...
    const recommendationDebug = isPlainObject(raw.recommendationDebug)
        ? raw.recommendationDebug
        : null;
    const recommendationUsed = recommendationDebug && isPlainObject(recommendationDebug.used)
        ? recommendationDebug.used
        : null;
    const recommendationStrategyRaw = String(raw.recommendationStrategy ?? '')
        .trim()
        .toLowerCase();
    const recommendationStrategy = recommendationStrategyRaw || (Boolean(recommendationUsed?.openai) ? 'openai' : 'tmdb');
    ...
    const googleSuggestedTitles = sortTitles(uniqueStrings(asStringArray(recommendationDebug?.googleSuggestedTitles)));
    const openAiSuggestedTitles = sortTitles(uniqueStrings(asStringArray(recommendationDebug?.openAiSuggestedTitles)));
    const tmdbSuggestedTitles = sortTitles(uniqueStrings(asStringArray(recommendationDebug?.tmdbSuggestedTitles)));
    const recommendationFacts = [];
    recommendationFacts.push({ label: 'Seed', value: seedTitle }, { label: 'Seed year', value: asNum(raw.seedYear) }, {
        label: 'Google',
        value: !googleEnabled
            ? 'Not enabled'
            : googleUsed
        ...
        label: 'TMDB',
        value: recommendationStrategy === 'tmdb'
            ? {
    ...
    const finalCollectionTitles = uniqueStrings(refresherCollectionFacts.flatMap((fact) => {
        if (!isPlainObject(fact.value))
            return [];
        return asStringArray(fact.value.items);
    }));
    const finalCollectionTitleSet = new Set(finalCollectionTitles.map((title) => normalizeTitle(title)));
    ...
    if (resolvedMissingFromFinal.length) {
        issues.push((0, job_report_v1_1.issue)('warn', `${resolvedMissingFromFinal.length} resolved title(s) were not found in the final chained collection snapshot.`));
    }
    const usedFallbackCollectionOrder = refresherCollectionFacts.some((fact) => {
        if (!isPlainObject(fact.value))
            return false;
        return fact.value.order === 'desired_fallback';
    });
    if (usedFallbackCollectionOrder) {
        issues.push((0, job_report_v1_1.issue)('warn', 'Chained refresher order snapshot used desired fallback for at least one library.'));
    }
    const tasks = [
        {
            id: 'recommendations',
            ...
            ? [
                {
                    ...
                    status: ctx.dryRun || !Boolean(radarr.enabled)
                        ? 'skipped'
                        : radarrFailed
                            ? 'failed'
                            : 'success',
                    facts: [
                        {
                            ...
                            value: {
                                ...
                                unit: 'movies',
                                items: sortTitles(uniqueStrings(radarrLists ? asStringArray(radarrLists.attempted) : [])),
                            },
                        },
                        {
                            label: 'Added',
                            value: {
                                ...
                                unit: 'movies',
                                items: sortTitles(uniqueStrings(radarrLists ? asStringArray(radarrLists.added) : [])),
                            },
                        },
                        {
                            label: 'Exists',
                            value: {
                                ...
                                unit: 'movies',
                                items: sortTitles(uniqueStrings(radarrLists ? asStringArray(radarrLists.exists) : [])),
                            },
                        },
                        {
                            label: 'Failed',
                            value: {
                                ...
                                unit: 'movies',
                                items: sortTitles(uniqueStrings(radarrLists ? asStringArray(radarrLists.failed) : [])),
                            },
                        },
                    ],
                },
            ...
            ? [
                {
                    ...
                    status: ctx.dryRun || !Boolean(sonarr.enabled)
                        ? 'skipped'
                        : sonarrFailed
                            ? 'failed'
                            : 'success',
                    facts: [
                        {
                            ...
                            value: {
                                ...
                                unit: 'shows',
                                items: sortTitles(uniqueStrings(sonarrLists ? asStringArray(sonarrLists.attempted) : [])),
                            },
                        },
                        {
                            label: 'Added',
                            value: {
                                ...
                                unit: 'shows',
                                items: sortTitles(uniqueStrings(sonarrLists ? asStringArray(sonarrLists.added) : [])),
                            },
                        },
                        {
                            label: 'Exists',
                            value: {
                                ...
                                unit: 'shows',
                                items: sortTitles(uniqueStrings(sonarrLists ? asStringArray(sonarrLists.exists) : [])),
                            },
                        },
                        {
                            label: 'Failed',
                            value: {
                                ...
                                unit: 'shows',
                                items: sortTitles(uniqueStrings(sonarrLists ? asStringArray(sonarrLists.failed) : [])),
                            },
                        },
                    ],
                },
            ...
            : []),
        {
            ...
            rows: [
                (0, job_report_v1_1.metricRow)({
                    label: 'Rows (total)',
                    start: totalBefore,
                    changed: totalBefore !== null && totalAfter !== null ? totalAfter - totalBefore : null,
                    end: totalAfter,
                    unit: 'rows',
                }),
            ],
        },
        {
            ...
            issues: refresherError ? [(0, job_report_v1_1.issue)('error', refresherError)] : undefined,
        },
    ];
        ...
        sections: [
            {
                ...
                title: 'Totals',
                rows: [
                    (0, job_report_v1_1.metricRow)({ label: 'Recommendations generated', end: generated, unit: 'titles' }),
                    (0, job_report_v1_1.metricRow)({ label: 'Resolved in Plex', end: resolvedInPlex, unit: 'items' }),
                    (0, job_report_v1_1.metricRow)({ label: 'Missing in Plex', end: missingInPlex, unit: 'titles' }),
                ],
            },
            {
                id: 'points',
                ...
                rows: [
                    ...
                    (0, job_report_v1_1.metricRow)({
                        label: 'Rows (active)',
                        start: activeBefore,
                        changed: activeBefore !== null && activeAfter !== null ? activeAfter - activeBefore : null,
                        end: activeAfter,
                        unit: 'rows',
                    }),
                    (0, job_report_v1_1.metricRow)({
                        label: 'Rows (pending)',
                        start: pendingBefore,
                        changed: pendingBefore !== null && pendingAfter !== null ? pendingAfter - pendingBefore : null,
                        end: pendingAfter,
                        unit: 'rows',
                    }),
                    (0, job_report_v1_1.metricRow)({
                    ...
                    (0, job_report_v1_1.metricRow)({ label: 'Decayed', end: points ? asNum(points.decayed) : null, unit: 'rows' }),
                    (0, job_report_v1_1.metricRow)({
                        label: 'Removed',
...
//# sourceMappingURL=immaculate-taste-collection.job.js.map