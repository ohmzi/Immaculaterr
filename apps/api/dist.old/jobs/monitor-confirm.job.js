...
exports.MonitorConfirmJob = void 0;
...
let MonitorConfirmJob = class MonitorConfirmJob {
    settingsService;
    plexServer;
    radarr;
    sonarr;
    constructor(settingsService, plexServer, radarr, sonarr) {
    ...
    async run(ctx) {
        ...
        const plexBaseUrl = pickString(settings, 'plex.baseUrl') ??
            pickString(settings, 'plex.url') ??
            requireString(settings, 'plex.baseUrl');
        const plexToken = pickString(secrets, 'plex.token') ??
            pickString(secrets, 'plexToken') ??
            requireString(secrets, 'plex.token');
        const radarrBaseUrl = pickString(settings, 'radarr.baseUrl') ??
        ...
        const sonarrBaseUrl = pickString(settings, 'sonarr.baseUrl') ??
        ...
        await ctx.patchSummary({ plex: summary.plex });
        ...
        for (const sec of movieSections) {
            ...
            void ctx.patchSummary({ plex: summary.plex }).catch(() => undefined);
        }
        ...
        await ctx.patchSummary({ radarr: summary.radarr });
        setProgress({
            step: 'radarr_scan',
        ...
        for (const movie of monitoredMovies) {
            ...
            if (radarrSample.length < 25)
                radarrSample.push(title);
            if (ctx.dryRun) {
                radarrUnmonitored += 1;
            }
            else {
            ...
            if (radarrChecked % 50 === 0 ||
                ...
                void ctx
                    .patchSummary({ radarr: summary.radarr })
                    .catch(() => undefined);
                setProgress({
                    step: 'radarr_scan',
                    message: 'Scanning Radarr monitored movies…',
        ...
        await ctx.patchSummary({ sonarr: summary.sonarr });
        setProgress({
            step: 'sonarr_scan',
        ...
        for (const series of monitoredSeries) {
            ...
            for (const [season, seasonEpisodes] of episodesBySeason.entries()) {
                ...
                const seasonEpisodesToUnmonitor = [];
                for (const ep of seasonEpisodes) {
                    ...
                    if (isMonitored)
                        sonarrEpisodesMonitoredBefore += 1;
                    if (isInPlex) {
            ...
            if (sonarrSeriesProcessed % 10 === 0 || sonarrSeriesProcessed === sonarrSeriesTotal) {
                ...
                void ctx
                    .patchSummary({ sonarr: summary.sonarr })
                    .catch(() => undefined);
                setProgress({
                    step: 'sonarr_scan',
                    message: 'Scanning Sonarr monitored series…',
...
exports.MonitorConfirmJob = MonitorConfirmJob;
exports.MonitorConfirmJob = MonitorConfirmJob = __decorate([
...
function buildMonitorConfirmReport(params) {
    ...
    const sonarrEpisodesMonitoredAfter = sonarrEpisodesMonitoredBefore !== null
        ? Math.max(0, sonarrEpisodesMonitoredBefore - sonarrEpisodesUnmonitored)
        : null;
    const sonarrSearchQueued = typeof sonarr.missingEpisodeSearchQueued === 'boolean'
        ? sonarr.missingEpisodeSearchQueued
        : null;
    const issues = [
        ...(radarrMissingTmdbId
            ? [
                (0, job_report_v1_1.issue)('warn', `Radarr: ${radarrMissingTmdbId} monitored movie(s) missing TMDB id (skipped).`),
            ]
            : []),
        ...(radarrSkippedPathConflicts
            ? [
                (0, job_report_v1_1.issue)('warn', `Radarr: ${radarrSkippedPathConflicts} unmonitor(s) skipped due to path conflicts.`),
            ]
            : []),
        ...(!ctx.dryRun && sonarrSearchQueued === false
            ? [(0, job_report_v1_1.issue)('warn', 'Sonarr: MissingEpisodeSearch was not queued.')]
            : []),
    ];
    return {
        ...
        sections: [
            {
                ...
                rows: [
                    ...
                    (0, job_report_v1_1.metricRow)({ label: 'TV libraries', end: plexTvLibraries, unit: 'libraries' }),
                    (0, job_report_v1_1.metricRow)({ label: 'TMDB ids indexed', end: asNum(plex.tmdbIds), unit: 'ids' }),
                    (0, job_report_v1_1.metricRow)({ label: 'TVDB shows indexed', end: asNum(plex.tvdbShows), unit: 'shows' }),
                ],
            },
            {
                id: 'radarr',
                ...
                rows: [
                    ...
                    (0, job_report_v1_1.metricRow)({ label: 'Skipped', end: radarrSkipped, unit: 'movies' }),
                ],
            },
            {
                id: 'sonarr',
        ...
        tasks: [
            ...
            },
            {
                ...
                issues: [
                    ...(radarrMissingTmdbId
                        ? [(0, job_report_v1_1.issue)('warn', `${radarrMissingTmdbId} missing TMDB id (skipped).`)]
                        : []),
                    ...(radarrSkippedPathConflicts
                        ? [(0, job_report_v1_1.issue)('warn', `${radarrSkippedPathConflicts} path conflicts.`)]
                        : []),
                ],
            },
            {
                ...
                rows: [
                    ...
                    (0, job_report_v1_1.metricRow)({ label: 'Episodes checked', end: asNum(sonarr.episodesChecked), unit: 'episodes' }),
                ],
            },
            {
                ...
                issues: !ctx.dryRun && sonarrSearchQueued === false
                    ? [(0, job_report_v1_1.issue)('error', 'Search was not queued.')]
                    : [],
            },
        ],
        issues,
...
//# sourceMappingURL=monitor-confirm.job.js.map