...
exports.PlexCuratedCollectionsService = void 0;
...
let PlexCuratedCollectionsService = class PlexCuratedCollectionsService {
    plexServer;
    constructor(plexServer) {
        this.plexServer = plexServer;
    }
    async rebuildMovieCollection(params) {
        const { ctx, baseUrl, token, machineIdentifier, movieSectionKey, collectionName, itemType = 1, desiredItems, randomizeOrder = false, pinCollections = true, pinTarget = 'admin', collectionHubOrder, preferredHubTargets = [], } = params;
        ...
        const normalizedTarget = (0, plex_collections_utils_1.normalizeCollectionTitle)(collectionName);
        for (const coll of allCollections) {
            const collTitle = coll.title || '';
            const collNormalized = (0, plex_collections_utils_1.normalizeCollectionTitle)(collTitle);
        ...
        let plexCollectionKey = null;
        let lastAddedTitle = null;
        let collectionItems = [];
        let collectionItemsSource = 'desired_fallback';
        let existingItems = [];
        let existingCount = 0;
        if (!ctx.dryRun) {
            if (deletedSuccessfully) {
        ...
        if (!plexCollectionKey) {
            ...
            try {
                ...
                await ctx.warn('collection: create with seeded item failed, retrying without seed item', {
                    collectionName,
                    movieSectionKey,
                    itemType,
                    seedRatingKey: first,
            ...
            try {
                ...
                existingKeys = new Set(itemsAfterCreate
        ...
        if (plexCollectionKey) {
            ...
            for (let i = 0; i < 10; i += 1) {
                try {
                    ...
                    const setsMatch = orderedKeys.length === desiredKeySet.size &&
            ...
            if (lastErr) {
                ...
                await ctx.warn('collection: using desired fallback order; Plex ordered snapshot did not settle in time', {
                    collectionName,
                    plexCollectionKey,
                    desiredCount: desired.length,
        ...
        if (pinCollections &&
            ...
            try {
                ...
                await this.pinCuratedCollectionHubs({
                    ...
                    mediaType,
                    pinTarget,
                    collectionHubOrder: collectionHubOrder ?? Array.from(plex_collections_utils_1.CURATED_MOVIE_COLLECTION_HUB_ORDER),
                    preferredHubTargets: [
                        ...preferredHubTargets,
    ...
    async setCollectionArtwork(params) {
        ...
        if (artworkPaths.poster) {
            try {
                ...
                await ctx.info('collection: poster set', {
                    collectionName,
                    poster: node_path_2.default.basename(artworkPaths.poster),
                });
            }
            catch (err) {
        ...
        if (artworkPaths.background) {
            try {
                ...
                await ctx.info('collection: background set', {
                    collectionName,
                    background: node_path_2.default.basename(artworkPaths.background),
                });
            }
            catch (err) {
    ...
    async pinCuratedCollectionHubs(params) {
        const { ctx, baseUrl, token, librarySectionKey, mediaType, pinTarget, collectionHubOrder, preferredHubTargets = [], } = params;
        const requestedOrder = (0, plex_collections_utils_1.sortCollectionNamesByCuratedBaseOrder)({
            collectionNames: collectionHubOrder,
            mediaType,
        });
        const targetVisibility = pinTarget === 'friends'
            ? {
        ...
        const resolveRequestedUserSuffix = (name) => {
            const match = String(name ?? '')
                .trim()
                .match(/\(([^)]+)\)\s*$/);
            return (0, plex_collections_utils_1.normalizeCollectionTitle)(match?.[1] ?? '');
        };
        const resolveHubTargets = (allCollections) => {
            ...
            for (const requestedCollectionName of requestedOrder) {
                const normalizedRequested = (0, plex_collections_utils_1.normalizeCollectionTitle)(requestedCollectionName);
                const exactMatch = availableCollections.find((item) => {
                    if (!item.ratingKey || usedCollectionKeys.has(item.ratingKey))
                        return false;
                    return (0, plex_collections_utils_1.normalizeCollectionTitle)(item.title) === normalizedRequested;
                });
                if (exactMatch) {
                ...
                const preferredMatch = preferredHubTargets.find((target) => {
                    ...
                    if (!preferredName)
                        return false;
                    if ((0, plex_collections_utils_1.normalizeCollectionTitle)(preferredName) === normalizedRequested) {
                        return true;
                    }
                    return (0, plex_collections_utils_1.hasSameCuratedCollectionBase)({
                        left: preferredName,
                ...
                const fallback = fallbackCandidates.sort((a, b) => {
                    ...
                    const aUserPriority = requestedUserSuffix && aSuffix === requestedUserSuffix ? 0 : 1;
                    const bUserPriority = requestedUserSuffix && bSuffix === requestedUserSuffix ? 0 : 1;
                    if (aUserPriority !== bUserPriority)
                        return aUserPriority - bUserPriority;
                    return (0, plex_collections_utils_1.normalizeCollectionTitle)(a.title).localeCompare((0, plex_collections_utils_1.normalizeCollectionTitle)(b.title));
                })[0];
                if (fallback) {
        ...
        let listAttempt = 0;
        let resolved = resolveHubTargets(await this.plexServer.listCollectionsForSectionKey({
            baseUrl,
            token,
            librarySectionKey,
            take: 500,
        ...
        while (resolved.missingCollections.length > 0 && listAttempt < 6) {
            listAttempt += 1;
            await sleep(400);
            resolved = resolveHubTargets(await this.plexServer.listCollectionsForSectionKey({
                baseUrl,
                token,
                librarySectionKey,
                take: 500,
            }));
            if (resolved.missingCollections.length === 0)
                break;
        }
        ...
        await ctx.info('hub_pin: resolved target collections', {
            ...
            matches: targetMatches.map((m) => ({
                ...
                baseName: (0, plex_collections_utils_1.resolveCuratedCollectionBaseName)({
        ...
        try {
            ...
            const resolveHubIdentifier = async (collectionKey) => {
    ...
    getArtworkPaths(collectionName) {
        const normalizedName = (0, plex_collections_utils_1.stripUserCollectionPrefix)(collectionName)
            .trim()
            .toLowerCase();
        const collectionArtworkMap = {
        ...
        const roots = [cwd, (0, node_path_1.join)(cwd, '..'), (0, node_path_1.join)(cwd, '..', '..'), (0, node_path_1.join)(cwd, '..', '..', '..')];
        const rels = [
            (0, node_path_1.join)('apps', 'web', 'src', 'assets', 'collection_artwork'),
            (0, node_path_1.join)('assets', 'collection_artwork'),
        ];
        const candidates = roots.flatMap((root) => rels.map((rel) => (0, node_path_1.join)(root, rel)));
        let assetsDir = null;
        ...
        const posterPng = (0, node_path_1.join)(assetsDir, 'posters', `${artworkName}.png`);
        const posterJpg = (0, node_path_1.join)(assetsDir, 'posters', `${artworkName}.jpg`);
        const poster = (0, node_fs_1.existsSync)(posterPng)
            ? posterPng
            : (0, node_fs_1.existsSync)(posterJpg)
                ? posterJpg
                : null;
        const backgroundPng = (0, node_path_1.join)(assetsDir, 'backgrounds', `${artworkName}.png`);
        const backgroundJpg = (0, node_path_1.join)(assetsDir, 'backgrounds', `${artworkName}.jpg`);
        const background = (0, node_fs_1.existsSync)(backgroundPng)
            ? backgroundPng
            : (0, node_fs_1.existsSync)(backgroundJpg)
                ? backgroundJpg
                : null;
        return { poster, background };
    }
};
exports.PlexCuratedCollectionsService = PlexCuratedCollectionsService;
exports.PlexCuratedCollectionsService = PlexCuratedCollectionsService = __decorate([
...
//# sourceMappingURL=plex-curated-collections.service.js.map