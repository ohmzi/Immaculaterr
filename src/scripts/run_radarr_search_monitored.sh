...
RADARR_URL=$(yq '.radarr.url' < "$CONFIG_FILE" | tr -d '"')
API_KEY=$(yq '.radarr.api_key' < "$CONFIG_FILE" | tr -d '"')
ROOT_FOLDER=$(yq '.radarr.root_folder' < "$CONFIG_FILE" | tr -d '"')
TAG_NAME=$(yq '.radarr.tag_name' < "$CONFIG_FILE" | tr -d '"')
...
if [[ -z "$RADARR_URL" || -z "$API_KEY" ]]; then
    log ERROR "Radarr URL or API Key missing in config.yaml"
    exit 1
fi
